...
import { ScanSearch, AlertCircle, AlertTriangle, Lightbulb } from "lucide-react";

const EnhancementTips = () => {
  const resumeData = useAppSelector(state => state.resume);
  const [tips, setTips] = useState<{text: string, type: 'info' | 'warning' | 'tip'}[]>([]);
    ...
    const generateTips = () => {
      ...
      if (resumeData.workExperiences.length === 0) {
        ...
        const actionVerbs = [
          ...
          'increased', 'negotiated', 'developed', 'led', 'organized', 'delivered'
        ];
